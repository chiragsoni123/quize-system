...
<body>
    ...
    <div class="bg-gray-100 flex flex-col items-center min-h-screen pt-5">
        ...
        <h1 class="text-4xl text-center text-green-800 font-bold mb-6 ">{{$quizName}}</h1>

       <h2 class="text-2xl text-center text-green-800 font-bold mb-6 ">Question no. {{session('currentQuiz')['totalMcq']}}</h2>
       <h2 class="text-2xl text-center text-green-800 font-bold mb-6 ">{{session('currentQuiz')['currentMcq']}} of {{session('currentQuiz')['totalMcq']}}</h2>
       ...
       <div class="mt-2 p-4 bg-white shadow-2xl rounded-xl w-150">
            ...
            <form action="/submit-next/{{$mcqData->id}}" class="space-y-4" method="post">
                ...
                <button type="submit" class="w-full bg-blue-500 rounded-xl px-4 py-2 cursor-pointer text-white">Submit your answer</button>
            </form>
       </div>
